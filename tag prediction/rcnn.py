...
class RCNN():
    def __init__(self, num_classes, vocab_size,
                 ...
                 context_size, max_sequence_length, l2_reg_lambda=0.0):
        self.X = tf.compat.v1.placeholder(
            tf.int32, shape=[None, None], name='input_X')
        ...
        self.y = tf.compat.v1.placeholder(
            tf.float32, shape=[None, num_classes], name='input_y')
        ...
        with tf.name_scope('recurrent'):
            clw1 = tf.Variable(tf.random.normal(
            ...
            crwn = tf.Variable(tf.random.normal(
            ...
            outputs, states = tf.nn.bidirectional_dynamic_rnn(
                lstm_cell, lstm_cell, self.embedded_chars, sequence_length=self.sequence_length, initial_state_fw=clw1, initial_state_bw=crwn, dtype=tf.float32)
        ...
        with tf.name_scope("dropout"):
            self.h_drop = tf.nn.dropout(self.y3, rate = 1-self.dropout_keep_prob)
        ...
        with tf.name_scope('output'):
            ...
            b4 = tf.Variable(tf.random.normal(
